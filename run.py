...
i = 36000
while(cv2.waitKey(10) != ord('q') and i<44000):
    file = os.path.exists("driving_dataset/" + str(i) + ".jpg")
    if(file):
        img1 = image.load_img("driving_dataset/" + str(i) + ".jpg",color_mode='rgb')
        img1 = image.img_to_array(img1)/255.0
        img = image.load_img("driving_dataset/" + str(i) + ".jpg",color_mode='rgb',target_size=[200,200])
        img = image.img_to_array(img)/255.0
        img_resh = np.reshape(img,[1,200,200,3])
